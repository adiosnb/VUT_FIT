...
**/
    if (RootPtr) {
        tBTNodePtr *current_leaf = RootPtr;
        //v tomto cykle sa najde ukazatel kde by mal byt vytvoreny novy prvook
        while (*current_leaf != NULL) {
            if ((*current_leaf)->Cont > Content) {
                current_leaf = &(*current_leaf)->LPtr;
            } else {
                if ((*current_leaf)->Cont == Content) {
                    return;
                } else {
                    current_leaf = &(*current_leaf)->RPtr;
                }
            }
        }

        //vytvorenie noveho prvku
        *current_leaf = malloc(sizeof(struct tBTNode));
        (*current_leaf)->Cont = Content;
        (*current_leaf)->LPtr = NULL;
        (*current_leaf)->RPtr = NULL;
    }
}
...
**/
    ...
    Leftmost_Preorder(current,&stack);
    while (!SEmptyP(&stack)){
...
**/
    ...
    Leftmost_Inorder(current,&stack);
    while (!SEmptyP(&stack)){
        current = STopPopP(&stack);
        BTWorkOut(current);
...
**/
    ...
    Leftmost_Postorder(current,&stackP,&stackB);

    while (!SEmptyP(&stackP)){
        left = STopPopB(&stackB);
        current = STopPopP(&stackP);

        if (left) {
            SPushB(&stackB,FALSE);
            SPushP(&stackP,current);
            Leftmost_Postorder(current->RPtr,&stackP,&stackB);
        } else {
            BTWorkOut(current);
        }
    }
...
**/
    ...
    if (RootPtr) {
        ...
        tBTNodePtr help_leaf = NULL, current_leaf = *RootPtr;

        do {
            if (current_leaf == NULL) {
                if (!SEmptyP(&stack)) {
                    current_leaf = STopPopP(&stack);
                }
            } else {
                if (current_leaf->RPtr != NULL) {
                    SPushP(&stack, current_leaf->RPtr);
                }
                help_leaf = current_leaf;
                current_leaf = current_leaf->LPtr;
                free(help_leaf);
            }
        } while (current_leaf != NULL || !SEmptyP(&stack));
        (*RootPtr) = NULL;
    }
}
