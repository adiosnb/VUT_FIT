	...
	if (RootPtr != NULL){
        if (RootPtr->Key == K){
            *Content = RootPtr->BSTNodeCont;
            ...
            return (BSTSearch(RootPtr->LPtr,K,Content) || BSTSearch(RootPtr->RPtr,K,Content));  //TODO check
        }
    } else {
        return FALSE;
    }
    ...
    if (RootPtr) {
        ...
        if (*RootPtr != NULL) {
            if ((*RootPtr)->RPtr != NULL) {
                ...
                del_item = *RootPtr;
                PtrReplaced->Key = del_item->Key;
                PtrReplaced->BSTNodeCont = del_item->BSTNodeCont;
                *RootPtr = del_item->LPtr;
                free(del_item);
            }
        }
    }
        ...
        if (*RootPtr != NULL) {
            if ((*RootPtr)->Key == K) {
                if ((*RootPtr)->LPtr != NULL && (*RootPtr)->RPtr != NULL) {
                    ...
                    *RootPtr = NULL;
                    if (del_list->LPtr != NULL) {
                        *RootPtr = del_list->LPtr;
                    }
                    if (del_list->RPtr != NULL) {
                        *RootPtr = del_list->RPtr;
                    }
                    free(del_list);
                }
            } else {
